...
if periodo_inicio:
    ...
    fig.update_layout(
        yaxis=dict(range=[0, 100])  # Define o eixo Y de 0 a 50
    )
    ...
    fig_bar.update_layout(
        title="Gráfico de Barras com Barra Inferior Centralizada",
        yaxis=dict(
            zeroline=True,
            zerolinewidth=1,
            zerolinecolor='grey'
        ),
        barmode='overlay'  # Para que as posições definidas sejam respeitadas
    )
    col1,col2 = st.columns([2,10])
    with col1:
        with st.container(border=True):
            st.markdown('**Total de Tempo:**')
            st.markdown(f"<h1 style='text-align: center; color: black;'>{math.floor(len(df_final) / 60)}:{len(df_final) % 60:02}</h1>",unsafe_allow_html=True)
            #:red[Texto Exemplo]
        with st.container(border=True):
            st.markdown('**Tempo Trabalhando:**')
            st.markdown(f"<h1 style='text-align: center; color: black;'>{math.floor(minutos_trabalhados / 60)}:{minutos_trabalhados % 60:02}</h1>",unsafe_allow_html=True)
        with st.container(border=True):
            st.markdown('**Ocupação média por Tempo trabalhando:**')
            st.markdown(f"<h1 style='text-align: center; color: black;'>{percentPerHoraTrab:.2f}%</h1>",unsafe_allow_html=True)
        with st.container(border=True):
            st.markdown('**Tempo Parado:**')
            st.markdown(f"<h1 style='text-align: center; color: black;'>{math.floor(minutos_parados / 60)}:{minutos_parados % 60:02}</h1>",unsafe_allow_html=True)
    with col2:
        
        st.markdown('## Indicativo de uso da linha de pintura da esteira no dia {} até {}'.format(read_date_ini.strftime('%d/%m/%Y'),read_date_fin.strftime('%d/%m/%Y')))
        st.date_input('Filtro Leitura',value=(datetime.today(),datetime.today()),key='periodo_tempo')
        ...
        st.markdown(f"<p style='text-align: right; color: black;'>Última consulta feita em {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}</h1>", unsafe_allow_html=True)
else:
    st.markdown(f'''
        ## Sem Dados disponíveis entre {read_date_ini.strftime('%d/%m/%Y')} até {read_date_fin.strftime('%d/%m/%Y')}
    ''')
    st.date_input('Filtro Leitura',value=(datetime.today(),datetime.today()),key='periodo_tempo')