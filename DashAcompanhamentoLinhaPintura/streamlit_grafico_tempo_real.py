...
def create_bar_graph(display_data,show_date_start,show_date_end):
    ...
    # dias_atras = ((show_date_end - timedelta(days=1)) - show_date_start).days
    # if dias_atras:
    #     min_total += dias_atras * 60 * (18 - 5)
    ...
    #print('{} - {} = {} ({})'.format(display_data['date'].max(),display_data['date'].min(),(display_data['date'].max() - display_data['date'].min()), min_total))
    min_trab = 0
    ...
    return fig_bar, min_total, min_trab, percent_trab_geral, min_parado
...
def create_bar_graph_wrapper(display_data,show_date_start,show_date_end, q):
    fig_bar,min_total,min_trab,percent_trab_geral,min_parado = create_bar_graph(display_data,show_date_start,show_date_end)
    q.put((fig_bar,min_total,min_trab,percent_trab_geral,min_parado))
...
query_turno = f'''SELECT TIME_FORMAT(t.TurnoProdutivoHrEntrada, '%H:%i') AS HoraMinuto
from turnoprodutivo t 
where t.TurnoProdutivoDiaSemana = dayofweek('{read_date_ini}')
order by t.TurnoProdutivoHrEntrada limit 1'''

periodo_inicio = pd.read_sql(query_turno, conn)

######## CONTINUAR CHECANDO SE TurnoProdutivoQtdeMinutos é atualizado dinâmicamente
#st.dataframe(periodo_inicio)
...
if periodo_inicio.empty:
    ...
    horario = datetime.strptime(periodo_inicio.iloc[0, 0],'%H:%M')
...
if periodo_inicio:
    ...
    thread_bar = Thread(target=create_bar_graph_wrapper, args=(display_data,read_date_ini,read_date_fin, q_bar))
    ...
    fig_bar, min_total, min_trab, percent_trab_geral, min_parado = q_bar.get()
